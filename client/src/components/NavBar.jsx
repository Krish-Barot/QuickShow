...
const NavBar = () => {
  ...
  return (
    <div className='fixed top-0 left-0 z-50 w-screen flex items-center justify-between px-6 md:px-16 lg:px-36 py-5 sm:gap-10'>
      ...
      <div className={`max-md:absolute max-md:top-0 max-md:left-0 max-md:font-medium max-md:text-lg sm:gap-3 z-50 flex flex-col md:flex-row items-center max-md:justify-center lg:gap-8 min-md:px-8 py-3 max-md:h-screen min-md:rounded-full backdrop:blur bg-black/70 md:bg-white/10 md:border border-gray-300/20 overflow-hidden transition-[width] duration-300 ${isOpen ? 'max-md:w-full' : 'max-md:w-0'}`}>
        <XIcon className='md:hidden absolute top-6 right-6 w-6 h-6 cursor-pointer' onClick={() => setIsOpen(!isOpen)} />
        <Link className='hover:text-red-400 transition duration-500' onClick={() => { scrollTo(0, 0); setIsOpen(false) }} to='/'>Home</Link>
